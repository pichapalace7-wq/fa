...
import { Candle, MarketMetrics, SniperSetup, AnalysisResult, OrderBlock, FairValueGap, LiquidityPool, TradeIdea, PerformanceStats, LearningInsight, AISelfReview, TradeState, TPValidationItem, IntelligentStatus } from './types';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Direct Browser WebSocket fallback for Static / GitHub Pages hosting
    const staticFallbackTimeout = setTimeout(() => {
      if (!isMounted || hasReceivedBackendData) return;
      try {
        directWs = new WebSocket('wss://ws.derivws.com/websockets/v3?app_id=1089');
        directWs.onopen = () => {
          if (!directWs || directWs.readyState !== WebSocket.OPEN) return;
          setConnectionStatus('CONNECTED');
          directWs.send(JSON.stringify({ ticks: 'frxXAUUSD', subscribe: 1 }));
          directWs.send(JSON.stringify({ ticks: '1HZ10V', subscribe: 1 }));
          directWs.send(JSON.stringify({ ticks: 'JD25', subscribe: 1 }));
        };
        directWs.onmessage = (evt) => {
          ...
          try {
            ...
            if (data.msg_type === 'tick' && data.tick) {
              ...
              const bidVal = data.tick.bid || quote;
              const askVal = data.tick.ask || quote;
              const epoch = data.tick.epoch;

              if (sym === 'frxXAUUSD') {
                setLastTickPriceGold(quote);
                setBidGold(bidVal);
                setAskGold(askVal);
                setLastTickTimeGold(epoch * 1000);
                setTicksCountGold(c => c + 1);
              } else if (sym === '1HZ10V') {
                setLastTickPriceVol(quote);
                setBidVol(bidVal);
                setAskVol(askVal);
                setLastTickTimeVol(epoch * 1000);
                setTicksCountVol(c => c + 1);
              } else if (sym === 'JD25') {
                setLastTickPriceJump(quote);
                setBidJump(bidVal);
                setAskJump(askVal);
                setLastTickTimeJump(epoch * 1000);
                setTicksCountJump(c => c + 1);
              }
            }
          } catch {
            // Ignore parse errors
        ...
        // Ignore ws initialization errors
      }
    }, 2500);
    ...
    return () => {
      ...
      clearTimeout(staticFallbackTimeout);
          ...
          <section id="panel-institutional-checklist" className="bg-slate-900/50 border border-slate-800 p-4 rounded-lg flex-1 flex flex-col">
            ...
            <div className="space-y-1.5 overflow-y-auto max-h-[380px] pr-1 flex-1">
              {[
                ...
                { label: 'M1 Break of Structure (BOS)', val: smcMetrics.confirmationChecklist.bos, desc: smcMetrics.confirmationChecklist.bos ? 'Confirmed' : 'No Break' },
                { label: 'M1 Change of Character (CHOCH)', val: smcMetrics.confirmationChecklist.choch, desc: smcMetrics.confirmationChecklist.choch ? 'Detected' : 'Neutral' },
