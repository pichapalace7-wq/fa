...
export function TrendPullbackRetestPanel({
  ...
  useEffect(() => {
    ...
    const isArmedInZone = activeSetup.executionState === 'ENTRY_ARMED' && !(activeSetup as any).entryBlockers?.hasBlockers && (activeSetup.proximity?.classification === 'INSIDE_ZONE' || activeSetup.proximity?.classification === 'OPTIMAL' || ((activeSetup.proximity as any)?.distanceToZone ?? 1) <= 0.05);
    const isValidatedClose = activeSetup.executionState === 'SETUP_VALIDATED' && !(activeSetup as any).entryBlockers?.hasBlockers && (activeSetup.proximity?.classification === 'INSIDE_ZONE' || activeSetup.proximity?.classification === 'OPTIMAL') && activeSetup.qualityScore >= 65;
              ...
              <div className="bg-slate-950/80 border border-slate-800 rounded-xl px-3.5 py-2 flex flex-wrap items-center justify-between gap-2 text-xs font-mono">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded bg-amber-950/80 text-amber-300 border border-amber-800/60 font-bold text-[10px] uppercase flex items-center gap-1">
                    ...
                    {(activeSetup.sessionIntelligence as any)?.isKillzone && ' (KILLZONE)'}
                  </span>
